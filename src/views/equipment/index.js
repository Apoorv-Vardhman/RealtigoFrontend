...
const Equipment = ()=>{
    ...
    const handleDelete = async (row, index) => {
        ...
        setRows(tempRows);
        await WebServices.axiosObject(account.token).delete(`equipments/${row._id}`)
            .then((response) => {
    ...
    useEffect(()=>{
       try {
           axios
               .get( configData.API_SERVER + 'equipments', { headers: { Authorization: `Bearer ${account.token}` } })
    ...
    return (
        <MainCard title="Equipment">
            <Grid container spacing={2} justifyContent="end" direction="row">
                <Button component={Link} to={{pathname: `/equipments/create`}}  variant="contained" color="primary">Create Equipment</Button>
                                ...
                                .map((row,index) => {
                                    return (
                                        <TableRow  hover role="checkbox" tabIndex={-1} >
                                            ...
                                            <TableCell align="left">
                                                <img src={configData.IMAGE_URL+row.banner} alt="N/A" height={80}  />
                                            </TableCell>
                                            <TableCell align="center">
                                                <Button component={Link} to={{pathname: `/equipments/${row._id}/edit`}}  variant="contained" color="primary">Edit</Button>

                                                <Button component={Link} to={{pathname: `/equipments/${row._id}/questions`}} startIcon={<RemoveRedEyeIcon />} variant="contained" color="success">Questions</Button>
