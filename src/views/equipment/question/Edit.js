...
const EquipmentQuestionEdit = (props)=>{
    ...
    const onSubmit = async data => {
        ...
        formData.append("equipment",props.id);
        await WebServices.axiosObject(account.token).put(`equipment-questions/${props.question}`,formData)
            .then((response) => {
                let responseData = response.data;

                toast(`${responseData.message}`, {position: "bottom-center", autoClose: 2000});
                history.push(`/equipments/${props.id}/questions`);
            })
            .catch((error) => {
    ...
    const fetchQuestion = async ()=>{
        await WebServices.axiosObject(account.token).get(`equipment-questions/${props.question}`)
            .then((response) => {
                let responseData = response.data;
                setValue("question",responseData.data.question);
                setValue("options",responseData.data.options);
    ...
    return (
        <MainCard title="Update Equipment Question">
...
export default EquipmentQuestionEdit
