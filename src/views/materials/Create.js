...
const MaterialCreate = ()=>{
    ...
    const onSubmit = async data => {
        ...
        setSubmitted(true);
        await WebServices.axiosObject(account.token).post("materials",formData)
            .then((response) => {
        ...
        try {
            axios
                ...
                .then(function (response) {
                    ...
                    {
                        console.log(d[0]._id);
                        setMaterialAttributes([{attribute:d[0]._id}])
                    }
                })
    ...
    return (
        <MainCard title="Add new Material">
                    ...
                    <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} style={{marginTop:"10px"}}>
                        <Grid item md={6} xs={12}>
                            <FormControl fullWidth>
                                ...
                                <Select {...register("attribute", { required: true })} value={row.attribute} error={Boolean(errors.attribute)}  labelId="select-attribute"   label="Category"
