...
const ServiceEdit = (props)=>{
    ...
    const onSubmit = async data => {
        ...
        setSubmitted(true)
        await WebServices.axiosObject(account.token).put(`services/${props.id}`,formData)
            .then((response) => {
    ...
    const getService = async ()=>{
        await WebServices.axiosObject(account.token).get(`services/${props.id}`)
            .then((response) => {
                let responseData = response.data;
                ...
                setValue('category', response.data.data.category, { shouldValidate: true })
    ...
    useEffect(()=>{
        try {
            axios
                .get( configData.API_SERVER + 'service-categories', { headers: { Authorization: `Bearer ${account.token}` } })
        ...
        <MainCard title="Add new Service">
            <form noValidate onSubmit={handleSubmit(onSubmit)}>
                <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                    ...
                    <Grid item md={6} xs={12}>
                        <FormControl fullWidth>
                            ...
                            <Select {...register("category", { required: true })} error={Boolean(errors.category)}  labelId="select-category" id="select-category" value={category} label="Category"
                ...
                <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                    ...
                    <Grid item md={6} xs={12}>
                        <label>Banner</label>
                        <input accept="image/*" {...register("banner")} className={classes.input} id="faceBanner" type="file" onChange={handleCaptureBanner}/>
