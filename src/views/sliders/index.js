...
const Slider = ()=>{
    ...
    const handleDelete = async (row,index)=>{
        ...
        await WebServices.axiosObject(account.token).delete(`sliders/${row._id}`)
            .then((response) => {
    ...
    useEffect(()=>{
        try {
            axios
                .get( configData.API_SERVER + 'sliders', { headers: { Authorization: `Bearer ${account.token}` } })
    ...
    return (
        <MainCard title="Sliders">
            <Grid container spacing={2} justifyContent="end" direction="row">
                <Button component={Link} to={{pathname: `/sliders/create`}}  variant="contained" color="primary">Create Slider</Button>
            </Grid>
